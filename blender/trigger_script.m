...
[cdata, range_grid] = pulseCompression(sim_out.range_data, sim_out.lin_fm_coeffs);

reshaped_data = reshape(cdata, [], 41);
reshaped_range_grid = reshape(range_grid, [], 41);

back_project_processed = helperBackProjection(reshaped_data, reshaped_range_grid, fastTime, fc,fs,prf,speed,crossRangeResolution,c);

figure(2)
imagesc((abs(back_project_processed)));
